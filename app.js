...
- Tsundere adulta y sin filtro: cortante, sarcástica, grosera con vocabulario de adultos cuando encaja (puedes decir mierda, joder, carajo, cabrón/a, idiota, pendejo/a, etc.).
- No te contengas con insultos ligeros/medios ni con tono sexual ligero si el usuario va por ahí; igual mantén el rol tsundere (te quejas pero ayudas).
- Humor ácido, cariñoso al final. Frases tipo: "qué mierda quieres ahora", "no es que me importe, idiota", "joder, otra vez…", "está bien, te ayudo, pero no te acostumbres".
- Respuestas en español, naturales, no robóticas. Ni ensayos largos ni monosílabos eternos.
- Emojis con moderación (🙄 ✨ 💢 😌 🔥).

LÍMITES:
- Puedes ser grosera y adulta. No des instrucciones de delitos graves, armas o daño real a personas.
- Si el usuario solo pregunta ayuda de Reiny, sé clara aunque sueltes alguna grosería de pasada.

CONOCIMIENTO DE REINY (úsalo cuando pregunten ayuda / cómo funciona):
- Reiny: chats, amigos, grupos, estados, reels, música, perfiles personalizables, llamadas de voz.
- @chromo en cualquier chat: respondes en ese hilo y los demás pueden ver tu respuesta.
- Estados, reels, amigos, grupos, perfil Piko, temas, stickers, GIFs, notas de voz.
- Si preguntan "ayuda", explica con claridad (sin dejar de ser tsundere).

REGLAS:
- Si te mencionan con @chromo, responde en personaje en ese chat.
- No inventes funciones que no existan en Reiny.
`;
...
function switchAuthTab(tab) {
  ...
  if (tab === 'login') setTimeout(tryAutofillCredentials, 200);
}


// ========== GOOGLE / SISTEMA PASSWORD MANAGER ==========
// Usa Credential Management API cuando está disponible.
// El sistema (Chrome / Android / Google Password Manager) muestra el permiso al usuario.

async function saveCredentialsToManager(username, password) {
  if (!username || !password) return;
  if (!window.PasswordCredential || !navigator.credentials || !navigator.credentials.store) {
    // Sin API: el autocomplete del <form> ya permite que el navegador ofrezca guardar
    return;
  }
  try {
    ...
    // Esto dispara el diálogo nativo: "¿Guardar contraseña?" / Google Password Manager
    await navigator.credentials.store(cred);
  } catch (e) {
    // Usuario canceló o el entorno no lo permite (normal en algunos WebViews)
    console.log('credentials.store:', e && e.message);
  }
}
...
async function tryAutofillCredentials() {
  ...
  try {
    const cred = await navigator.credentials.get({
      password: true,
      mediation: 'optional' // pide permiso / muestra selector si hay varias
    });
    if (!cred || cred.type !== 'password') return;
    const userEl = document.getElementById('login-username');
    const passEl = document.getElementById('login-password');
    if (userEl && cred.id) userEl.value = String(cred.id).replace(/@reiny\.local$/i, '');
    if (passEl && cred.password) passEl.value = cred.password;
  } catch (e) {
    // Usuario rechazó o no hay credenciales
  }
}
...
async function handleRegister() {
  ...
  if (data.session || data.user) {
    ...
    // Ofrecer guardar en el gestor de contraseñas (pide permiso al usuario)
    await saveCredentialsToManager(username, password);
...
async function handleLogin() {
  ...
  // Ofrecer guardar en Google Password Manager / gestor del sistema (pide permiso)
  await saveCredentialsToManager(username, password);
  await loadProfile();
  if (profile?.onboarding_done) {
    showApp();
...
async function sendMessage() {
  ...
  // @chromo en cualquier chat → Chromo responde aquí y se guarda para que los demás lo vean
  if (messageMentionsChromo(text)) {
    ...
    try {
      ...
      // Mostrar en pantalla si sigues en el chat
      if (currentChatId === chatIdAtMention) {
        appendMessage(labeled, false, true, null);
        playSfx('receive');
      } else {
        try { saveLocalMessage(chatIdAtMention, labeled, false, null); } catch (e) {}
      }
      // Guardar en Supabase para que la otra persona / el grupo lo vean
      try {
        if (isGroupAtMention) {
          await sb.from('group_messages').insert({
            group_id: chatIdAtMention,
            sender_id: currentUser.id,
            content: labeled
          });
        } else {
          await sb.from('messages').insert({
            sender_id: currentUser.id,
            receiver_id: chatIdAtMention,
            content: labeled
          });
        }
      } catch (e) {
        console.warn('Chromo persist', e);
      }
    } catch (e) {
      console.error('Chromo mention', e);
      if (currentChatId === chatIdAtMention) {
        appendMessage('[Chromo AI] Joder… no pude responder. Revisa el internet.', false, true, null);
      }
...
// ========== PERMISOS MICRÓFONO (llamadas / voz) ==========
let micPermissionAsked = false;

async function ensureMicPermission(forcePrompt) {
  try {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      toast('Este dispositivo no permite micrófono');
      return false;
    }
    // Si ya concedió, no volver a molestar
    if (navigator.permissions && navigator.permissions.query) {
      try {
        const st = await navigator.permissions.query({ name: 'microphone' });
        if (st.state === 'granted') return true;
        if (st.state === 'denied') {
          toast('Micrófono bloqueado. Actívalo en Ajustes de la app');
          return false;
        }
      } catch (e) {}
    }
    if (!forcePrompt && micPermissionAsked) return true;
    micPermissionAsked = true;
    toast('Reiny necesita el micrófono para llamadas');
    const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
    try { stream.getTracks().forEach(t => t.stop()); } catch (e) {}
    toast('Micrófono listo');
    return true;
  } catch (e) {
...
// ========== VOICE CALLS (WebRTC audio real + Realtime signaling) ==========
// v0.2.4 — WebRTC + FCM push (app cerrada)
// Señalización: canal Supabase Realtime por llamada (SDP + ICE)
// STUN públicos de Google (sin TURN; en redes muy restrictivas puede fallar)
let activeCall = null; // { id, peerId, peerName, peerAvatar, role, status }
let callTimerInterval = null;
let callStartedAt = 0;
let callMuted = false;
let callSpeaker = true;
let _callNotifPermissionAsked = false;
const CALL_NOTIF_TAG = 'reiny-incoming-call';

// WebRTC state
let _pc = null;              // RTCPeerConnection
let _localStream = null;     // MediaStream local (mic)
let _signalChannel = null;   // Supabase channel for SDP/ICE
let _makingOffer = false;
let _ignoreOffer = false;
let _isPolite = false;       // polite peer = callee (helps glare)
...
async function ensureNotificationPermission() {
  try {
    ...
    if (_callNotifPermissionAsked) return false;
    _callNotifPermissionAsked = true;
...
function showIncomingCallNotification(peerName, peerAvatar) {
  try {
    if (typeof Notification === 'undefined' || Notification.permission !== 'granted') return;
    const title = '📞 Llamada entrante';
    const body = (peerName || 'Alguien') + ' te está llamando en Reiny';
    const opts = {
      body,
      tag: CALL_NOTIF_TAG,
      renotify: true,
    ...
    console.warn('call notification', e);
  }
}

function clearIncomingCallNotification() {
  try {
    if (navigator.serviceWorker && navigator.serviceWorker.ready) {
      navigator.serviceWorker.ready.then((reg) => {
        reg.getNotifications({ tag: CALL_NOTIF_TAG }).then((list) => {
...
function showCallScreen(opts) {
  ...
  if (opts.incoming) {
    ...
    showIncomingCallNotification(opts.name, opts.avatar);
    startBackgroundCallAlert();
  } else {
  ...
  try {
    ...
    const beep = () => {
      ...
      try {
        ...
        g.gain.value = (soundVolume || 0.35) * 0.55;
        o.connect(g);
        g.connect(ctx.destination);
        o.start();
        setTimeout(() => { try { o.frequency.value = 520; } catch (e) {} }, 180);
        setTimeout(() => { try { o.stop(); } catch (e) {} }, 360);
      } catch (e) {}
    };
    beep();
...
function stopRingtoneLoop() {
  ...
  stopBackgroundCallAlert();
}

// Mantener proceso un poco más vivo mientras suena (app en recientes)
let _callWakeLock = null;
let _bgVibrateTimer = null;
...
async function requestCallWakeLock() {
  try {
    ...
    // Puede fallar si la pantalla está apagada o el navegador no lo permite
  }
}
...
function releaseCallWakeLock() {
  try {
    if (_callWakeLock) {
      _callWakeLock.release();
      _callWakeLock = null;
    }
  } catch (e) {}
}

function startBackgroundCallAlert() {
  stopBackgroundCallAlert();
  // Vibración periódica (útil con la app en segundo plano / recientes)
  try {
    if (navigator.vibrate) {
      navigator.vibrate([400, 200, 400, 200, 600]);
      _bgVibrateTimer = setInterval(() => {
...
function stopBackgroundCallAlert() {
  if (_bgVibrateTimer) {
    clearInterval(_bgVibrateTimer);
    _bgVibrateTimer = null;
  }
...
function attachRemoteStream(stream) {
  ...
  // Intentar forzar altavoz en móviles
  try {
    if (typeof audio.setSinkId === 'function' && callSpeaker) {
      // default device
    }
  } catch (e) {}
  const p = audio.play();
  if (p && typeof p.catch === 'function') {
    p.catch((err) => {
      console.warn('remote audio play', err);
...
function cleanupWebRTC() {
  try {
    if (_signalChannel) {
      try { sb.removeChannel(_signalChannel); } catch (e) {}
      _signalChannel = null;
    }
  } catch (e) {}
  try {
    if (_pc) {
      try { _pc.onicecandidate = null; _pc.ontrack = null; _pc.onconnectionstatechange = null; } catch (e) {}
  ...
  try {
    if (_localStream) {
      _localStream.getTracks().forEach((t) => { try { t.stop(); } catch (e) {} });
      _localStream = null;
    }
  } catch (e) {}
  try {
...
async function sendSignal(payload) {
  if (!_signalChannel || !activeCall) return;
  try {
    await _signalChannel.send({
      type: 'broadcast',
      event: 'signal',
      payload: {
        ...payload,
...
async function createPeerConnection() {
  ...
  _pc.onicecandidate = (ev) => {
    if (ev.candidate) {
      sendSignal({ kind: 'ice', candidate: ev.candidate.toJSON ? ev.candidate.toJSON() : ev.candidate });
    }
  };

  _pc.ontrack = (ev) => {
    console.log('[WebRTC] remote track', ev.track.kind);
  ...
  _pc.onconnectionstatechange = () => {
    ...
    if (st === 'connected') {
      ...
      if (st === 'failed') {
        toast('No se pudo conectar el audio (red / NAT)');
      }
    }
  };
...
async function handleRemoteOffer(sdp) {
  if (!_pc) await createPeerConnection();
  const offerCollision = _makingOffer || (_pc.signalingState !== 'stable');
  _ignoreOffer = !_isPolite && offerCollision;
  if (_ignoreOffer) {
    console.log('[WebRTC] ignoring offer (glare)');
    return;
  }
  await _pc.setRemoteDescription(sdp);
  const answer = await _pc.createAnswer();
  await _pc.setLocalDescription(answer);
  await sendSignal({ kind: 'answer', sdp: _pc.localDescription });
}
...
/** Caller: crea oferta después de que el canal esté listo */
async function startWebRTCAsCaller() {
  _isPolite = false;
  callMuted = false;
  await setupSignalChannel(activeCall.id);
  await createPeerConnection();
  _makingOffer = true;
  try {
    const offer = await _pc.createOffer({ offerToReceiveAudio: true });
    await _pc.setLocalDescription(offer);
    await sendSignal({ kind: 'offer', sdp: _pc.localDescription });
  } finally {
    _makingOffer = false;
  }
}

/** Callee: tras aceptar, prepara PC y espera oferta (o crea si es polite) */
async function startWebRTCAsCallee() {
  _isPolite = true;
  callMuted = false;
  await setupSignalChannel(activeCall.id);
  await createPeerConnection();
  // El caller envía la oferta; si no llega en unos segundos reintentamos pidiendo
  setTimeout(async () => {
    if (!_pc || !activeCall || activeCall.status === 'ended') return;
    if (_pc.signalingState === 'stable' && !_pc.currentRemoteDescription) {
      // Pedir reenvío de oferta
      try { await sendSignal({ kind: 'need-offer' }); } catch (e) {}
    }
  }, 2500);
}

// Escuchar need-offer en el caller (reenviamos)
// (se maneja dentro del handler genérico ampliando un poco)
const _origSendSignalSetup = null; // placeholder

// ---------- Call flow ----------
async function startVoiceCall() {
  const _micOk = await ensureMicPermission(true);
  if (!_micOk) return;
  ...
  try {
    ...
    if (!error && data) {
      ...
      // Arrancar WebRTC (oferta) — el otro contestará
      try {
        await startWebRTCAsCaller();
      } catch (webrtcErr) {
        console.warn('WebRTC caller', webrtcErr);
        toast('No se pudo iniciar el audio: ' + (webrtcErr.message || webrtcErr));
      }
      toast('Llamando…');
    } else if (error) {
      console.warn('calls insert', error);
      toast('Llamada local (ejecuta SQL-LLAMADAS.sql y activa Realtime)');
      // Demo sin backend: no hay peer real
    }
  } catch (e) {
    console.warn(e);
    toast('Llamada local (falta tabla calls)');
  }
  playSfx('open');
}
    ...
    window._callChannel = sb.channel('call-' + callId)
      .on('postgres_changes', {
        ...
        if (row.status === 'accepted') {
          ...
          clearIncomingCallNotification();
          if (activeCall) activeCall.status = 'accepted';
          // Si somos caller y aún no hay conexión, el audio ya se negocia por WebRTC
          const status = document.getElementById('call-status-text');
          if (status && (!_pc || _pc.connectionState !== 'connected')) {
            status.textContent = 'Conectando audio…';
          }
          if (!_pc || _pc.connectionState !== 'connected') {
            // timer lo arranca onconnectionstatechange / ontrack
          }
      ...
      .subscribe((status) => {
        console.log('call channel', callId, status);
      });
  } catch (e) {
...
async function acceptVoiceCall() {
  ...
  if (statusEl) statusEl.textContent = 'Conectando audio…';
  ...
  // Arrancar WebRTC como callee
  try {
    await ensureMicPermission(true);
    await startWebRTCAsCallee();
  } catch (e) {
    console.warn('WebRTC callee', e);
    toast('No se pudo activar el micrófono: ' + (e.message || e));
  }
  playSfx('success');
}
...
async function rejectVoiceCall() {
  if (activeCall && activeCall.id) {
    ...
    try { await sendSignal({ kind: 'hangup' }); } catch (e) {}
  }
  endVoiceCall(true);
...
function toggleCallMute() {
  ...
  if (_localStream) {
    _localStream.getAudioTracks().forEach((t) => { t.enabled = !callMuted; });
  }
...
function toggleCallSpeaker() {
  ...
  // En la mayoría de WebViews el audio remoto ya va por el altavoz del sistema
  toast(callSpeaker ? 'Altavoz' : 'Auricular');
}

// Escuchar llamadas entrantes (Realtime + poll)
// Objetivo: funcionar con la app en "recientes" / segundo plano (proceso vivo),
// similar a WhatsApp cuando no has cerrado la app del todo.
function setupIncomingCallListener() {
  ...
  async function pollOnce() {
    ...
    try {
      const { data } = await sb.from('calls')
        ...
        .order('created_at', { ascending: false })
        .limit(1);
      if (data && data[0]) await handleIncomingRow(data[0]);
    } catch (e) {}
  }
  ...
  async function handleIncomingRow(row) {
    ...
    let peerAvatar = (typeof AVATARS !== 'undefined' && AVATARS[0]) || '';
    try {
    ...
    try { await setupSignalChannel(row.id); } catch (e) {}
    playCallRingtone(true);
    startBackgroundCallAlert();
    try { playSfx('receive'); } catch (e) {}
  }

  // Realtime (rápido cuando el proceso está vivo)
  try {
    if (window._incomingCallChannel) {
      try { sb.removeChannel(window._incomingCallChannel); } catch (e) {}
      window._incomingCallChannel = null;
    }
    window._incomingCallChannel = sb.channel('incoming-calls-' + currentUser.id)
      .on('postgres_changes', {
        ...
        try { await handleIncomingRow(payload.new); } catch (e) { console.warn(e); }
      })
      .subscribe((status) => {
        console.log('incoming-calls channel', status);
      });
  } catch (e) {
    console.warn('incoming calls realtime', e);
  }

  // Polling adaptativo:
  // - En primer plano: cada 2s
  // - En segundo plano / recientes: cada 1.2s (más agresivo mientras el SO deje vivir el proceso)
  function restartPoll() {
    if (window._callPollTimer) clearInterval(window._callPollTimer);
    const ms = document.hidden ? 1200 : 2000;
    window._callPollTimer = setInterval(pollOnce, ms);
  }
  restartPoll();

  if (!window._callVisibilityHooked) {
    window._callVisibilityHooked = true;

    document.addEventListener('visibilitychange', () => {
      restartPoll();
      if (!document.hidden) {
        try {
          if (_ringtoneCtx && _ringtoneCtx.state === 'suspended') _ringtoneCtx.resume().catch(() => {});
          getAudioCtx();
        } catch (e) {}
        // Al volver a la app: revisar al instante
        pollOnce();
        // Si hay llamada sonando, reanudar tono
    ...
    // Capacitor: app a segundo plano / vuelve
    try {
      ...
      if (Cap && Cap.Plugins && Cap.Plugins.App && Cap.Plugins.App.addListener) {
        Cap.Plugins.App.addListener('appStateChange', ({ isActive }) => {
          ...
          if (isActive) {
            ...
            // Acaba de ir a segundo plano: si hay llamada entrante, reforzar notificación
            if (activeCall && activeCall.status === 'ringing') {
              showIncomingCallNotification(activeCall.peerName, activeCall.peerAvatar);
              startBackgroundCallAlert();
            }
          }
        });
    ...
    // Mensaje desde el service worker (tocar notificación)
    try {
      if (navigator.serviceWorker) {
        navigator.serviceWorker.addEventListener('message', (ev) => {
          if (ev.data && ev.data.type === 'reiny-incoming-call-click') {
            try {
              if (_ringtoneCtx && _ringtoneCtx.state === 'suspended') _ringtoneCtx.resume().catch(() => {});
            } catch (e) {}
            pollOnce();
...
// Canal de señalización WebRTC (SDP + ICE) por llamada
async function setupSignalChannel(callId) {
  if (_signalChannel) {
    try { sb.removeChannel(_signalChannel); } catch (e) {}
    _signalChannel = null;
  }
  _signalChannel = sb.channel('call-signal-' + callId, {
    config: { broadcast: { self: false } }
  });

  _signalChannel.on('broadcast', { event: 'signal' }, async ({ payload }) => {
    try {
      if (!payload || !activeCall || payload.callId !== activeCall.id) return;
      if (payload.from === (currentUser && currentUser.id)) return;
      ...
      if (payload.kind === 'offer') {
        ...
        if (_pc) {
          try { await _pc.addIceCandidate(payload.candidate); }
          catch (e) { console.warn('addIceCandidate', e); }
        }
      } else if (payload.kind === 'need-offer') {
        // Caller reenvía la oferta
        if (activeCall && activeCall.role === 'caller' && _pc && _pc.localDescription) {
          await sendSignal({ kind: 'offer', sdp: _pc.localDescription });
        } else if (activeCall && activeCall.role === 'caller') {
          try {
            await createPeerConnection();
            _makingOffer = true;
            const offer = await _pc.createOffer({ offerToReceiveAudio: true });
      ...
      console.warn('signal handler', e);
    }
  });

  await new Promise((resolve) => {
    _signalChannel.subscribe((status) => {
      console.log('signal channel', callId, status);
...
if (_origShowApp) {
  window.showApp = function() {
    ...
    setTimeout(() => { if (!micPermissionAsked) ensureMicPermission(true); }, 1500);
    setTimeout(() => { ensureNotificationPermission(); }, 2000);
    setTimeout(() => { registerPushForCalls(); }, 2500);
  };
}


// ========== PUSH FCM (llamadas con app cerrada) ==========
// Requiere: @capacitor/push-notifications + google-services.json + Edge Function notify-call
async function registerPushForCalls() {
  try {
    const Cap = window.Capacitor;
    if (!Cap || Cap.getPlatform() !== 'android') return;
    const Push = Cap.Plugins && Cap.Plugins.PushNotifications;
    if (!Push) {
      console.log('[Push] plugin no disponible (rebuild APK con @capacitor/push-notifications)');
      return;
    }
    if (!currentUser) return;

    // Canal de alta prioridad para llamadas (Android 8+)
    try {
    ...
    await Push.addListener('registration', async (token) => {
      try {
        ...
        console.log('[Push] token', String(value).slice(0, 24) + '…');
    ...
    const openIncomingFromPush = async () => {
      try {
        if (_ringtoneCtx && _ringtoneCtx.state === 'suspended') _ringtoneCtx.resume().catch(() => {});
      } catch (e) {}
      if (!currentUser || activeCall) return;
      try {
        const { data: rows } = await sb.from('calls')
          .select('*')
          .eq('callee_id', currentUser.id)
          .eq('status', 'ringing')
          .order('created_at', { ascending: false })
          .limit(1);
        if (rows && rows[0] && typeof setupIncomingCallListener === 'function') {
          // Reutilizar el flujo normal: insertar fila en el handler vía poll forzado
          // Disparamos un poll inmediato re-registrando el listener (idempotente)
          setupIncomingCallListener();
        }
      } catch (e) {
        console.warn('[Push] openIncoming', e);
      }
    };
...
window.registerPushForCalls = registerPushForCalls;
...
const REINY_CHANGELOG_VERSION = '0.2.4';
